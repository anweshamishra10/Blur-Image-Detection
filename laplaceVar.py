...
train_clear_images_dir = "C:/CERTH_ImageBlurDataset/TrainingSet/Undistorted"
train_blur_images_dir1 = "C:/CERTH_ImageBlurDataset/TrainingSet/Naturally-Blurred"
train_blur_images_dir2 = "C:/CERTH_ImageBlurDataset/TrainingSet/Artificially-Blurred"
val_digital_images_dir = "C:/CERTH_ImageBlurDataset/EvaluationSet/DigitalBlurSet"
val_natural_images_dir = "C:/CERTH_ImageBlurDataset/EvaluationSet/NaturalBlurSet"
...
for i in os.listdir(train_clear_images_dir):
    ...
    clear_max_laplacian.append(laplacian.max())
    clear_var_laplacian.append(laplacian.var())

print("Processing of clear images for training done...")


blurred_max_laplacian = []
blurred_var_laplacian = []
for i in os.listdir(train_blur_images_dir1):
    ...
    blurred_max_laplacian.append(laplacian.max())
    blurred_var_laplacian.append(laplacian.var())
...
labels = np.append(np.zeros(len(clear_max_laplacian)), np.ones(len(blurred_max_laplacian)))

laplacian_max = clear_max_laplacian + blurred_max_laplacian
laplacian_var = clear_var_laplacian + blurred_var_laplacian
...
validation_data1 = pd.read_excel("C:/CERTH_ImageBlurDataset/EvaluationSet/DigitalBlurSet.xlsx")
validation_data2 = pd.read_excel("C:/CERTH_ImageBlurDataset/EvaluationSet/NaturalBlurSet.xlsx")
...
val_clear_max_laplacian = []
val_clear_var_laplacian = []

val_blur_max_laplacian = []
val_blur_var_laplacian = []

for i in natural_clear_images:
    image = Image.open(os.path.join(val_natural_images_dir, i)).convert('L')
    laplacian = process_laplacian(image)
    
    val_clear_max_laplacian.append(laplacian.max())
    val_clear_var_laplacian.append(laplacian.var())
...
for i in natural_blur_images:
    ...
    val_blur_max_laplacian.append(laplacian.max())
    val_blur_var_laplacian.append(laplacian.var())
...
val_laplacian_max = val_clear_max_laplacian + val_blur_max_laplacian
val_laplacian_var = val_clear_var_laplacian + val_blur_var_laplacian

print("Saving the validation data in validation.csv file")

labels = np.append(np.zeros(len(val_clear_max_laplacian)), np.ones(len(val_blur_max_laplacian)))
